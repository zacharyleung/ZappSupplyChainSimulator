...
//				.getPolicy());
	...
	private static void simulateScenario(
		...
		try(PrintStream out = new PrintStream(outputFile)) {
			...
			out.printf("AccessParameter,SupplyDemandRatio,RandomSeed,Variable,Value%n");
			System.out.println("ZambiaPerformance.simulateScenario()");
			System.out.printf("scenario            = %s\n", scenario.getName());
			for (double accessParameter : phiParameters) {
				...
				for (int r = 0; r < NUMBER_OF_REPLICATIONS; ++r) {
					...
					SupplyChain supplyChain = 
							scf.getSupplyChainWithAccessParameter(
									supplyDemandRatio, accessParameter);
					...
					try {
						...
						System.out.println("Total time elapsed = " + Exp12.stopwatch);
						throw e;
					}
					...
					out.printf("%.2f,%.2f,%d,%s,%.4f%n",
							accessParameter, esdr, randomSeed,
